...
public class SubDomainAdapter extends RecyclerView.Adapter<SubDomainAdapter.MyView> {

    private ArrayList<HashMap<String, String>> arrayList;

    private Context mContext;

    public SubDomainAdapter(Context context, ArrayList<HashMap<String, String>> arrayList) {
        mContext = context;
      ...
      /*  Glide.with(mContext).load(arrayList.get(position).get("thumb_img")).listener(new RequestListener<String, GlideDrawable>() {
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                mContext.startActivity(new Intent(mContext, CourseDetailActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK).putExtra("id",_id).putExtra("title",_name).putExtra("description",arrayList.get(position).get("description")).putExtra("image",arrayList.get(position).get("thumb_img")));
            }
        });
    }
